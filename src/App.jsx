...
const RINGTONES = TRACKS.map((t, i) => ({
  ...
  photoUrl: t.photoUrl,
  gender: t.gender,
  demographic: t.gender ? `${t.gender} vocalist` : "",
  favorite: t.favorite,
...
const EBOOK = {downloadUrl: "/integrity-records-music-contract-guide.pdf",
  ...
  price: 8.99,
};const EBOOK_KINGDOMS = { downloadUrl: "/kingdoms-for-a-song.pdf", id: "ebook-kingdoms", type: "ebook",
  title: "Kingdoms for a Song", blurb: "A devotional exploring the temptation of Christ and the pressures artists face to compromise faith for fame.", price: 12.99,
  buyUrl: "https://buy.stripe.com/test_4gM5kC7j0fAS8aJdHA2go0a" };

const EBOOK_WILDERNESS = { downloadUrl: "/the-wilderness-deal.pdf", id: "ebook-wilderness", type: "ebook",
  title: "The Wilderness Deal", blurb: "What the industry offers in the desert, and what it costs to say yes.", price: 11.99,
  buyUrl: "https://buy.stripe.com/test_eVq7sKcDk4We9eN5b42go09" };

const EBOOK_BOWING = { downloadUrl: "/bowing-for-the-beat.pdf", id: "ebook-bowing", type: "ebook",
  title: "Bowing for the Beat", blurb: "A workbook for artists who want to know who they're really performing for.", price: 11.99,
  buyUrl: "https://buy.stripe.com/test_28EaEWbzg0FYcqZ4702go08" };

const EBOOK_GLORY = { downloadUrl: "/causing-the-glory-to-fall.pdf", id: "ebook-glory", type: "ebook",
  title: "Causing the Glory to Fall", blurb: "What 2 Chronicles 5 teaches musicians about worship God answers.", price: 10.99,
  buyUrl: "https://buy.stripe.com/test_dRm28qfPw4We3Utbzs2go07" };

const EBOOK_UNDIVIDED = { downloadUrl: "/undivided.pdf", id: "ebook-undivided", type: "ebook",
  title: "Undivided", blurb: "Loving God with all your heart in an industry built to divide it.", price: 9.99,
  buyUrl: "https://buy.stripe.com/test_9B628q1YG60i2Qp8ng2go05" };

const EBOOK_ASYOURSELF = { downloadUrl: "/as-yourself.pdf", id: "ebook-asyourself", type: "ebook",
  title: "As Yourself", blurb: "Loving your neighbor in an industry built on competition.", price: 9.99,
  buyUrl: "https://buy.stripe.com/test_bJecN446OfAS62B4702go04" };

const EBOOK_BONDOFPEACE = { downloadUrl: "/bond-of-peace.pdf", id: "ebook-bondofpeace", type: "ebook",
  title: "Bond of Peace", blurb: "Guarding the unity of the Spirit against the pattern that divides.", price: 10.99,
  buyUrl: "https://buy.stripe.com/test_bJefZg6eW4We4Yx8ng2go03" };

const EBOOK_OVERCOMERS = { downloadUrl: "/the-overcomers.pdf", id: "ebook-overcomers", type: "ebook",
...
async function saveLibrary(uid, lib) {
  if (!uid) return;
  try {
    await setDoc(doc(db, "users", uid), lib, { merge: true });
  } catch {}
}
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .ir-guide-pulse { animation: irGuidePulse 1.4s ease-in-out infinite; position: relative; }
      @keyframes irGuidePulse { 0%,100% { box-shadow: 0 0 0 0 ${COLORS.brass}99; } 50% { box-shadow: 0 0 0 9px ${COLORS.brass}00; } }
      .ir-guide-bounce { animation: irGuideBounce 1s ease-in-out infinite; }
      @keyframes irGuideBounce { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
    `}</style>
  );
}
  ...
  return (
    <div
      style={{
        ...
        pointerEvents: "none",
        whiteSpace: "nowrap",
        ...(position === "bottom" ? { top: "100%", marginTop: 10 } : { bottom: "100%", marginBottom: 10 }),
      }}
    >
      {position === "bottom" && (
        <span className="ir-guide-bounce" style={{ fontSize: 16, color: COLORS.brass, marginBottom: 2 }}>▲</span>
      )}
      <span
        className="ir-mono"
        style={{ background: COLORS.brass, color: COLORS.ink, padding: "5px 11px", borderRadius: 8, fontSize: 11, fontWeight: 700 }}
      >
        {text}
      </span>
      {position === "top" && (
        <span className="ir-guide-bounce" style={{ fontSize: 16, color: COLORS.brass, marginTop: 2 }}>▼</span>
      )}
    </div>
  );
}
...
function AuthForm({ onAuthed, title, subtitle }) {
  ...
  return (
    <div>
      {title && <h3 className="ir-display text-xl mb-1" style={{ color: COLORS.parchment }}>{title}</h3>}
      {subtitle && <p className="ir-body text-sm mb-4" style={{ color: "#9aa0b4" }}>{subtitle}</p>}

      <div className="flex rounded-md overflow-hidden mb-4 w-fit" style={{ border: `1px solid ${COLORS.line}` }}>
        <button
          type="button"
          onClick={() => { setMode("signup"); setError(""); }}
          className="ir-mono text-xs uppercase tracking-wider px-4 py-2"
          style={{ background: mode === "signup" ? COLORS.brass : "transparent", color: mode === "signup" ? COLORS.ink : COLORS.offwhite }}
        >
          Sign up
        </button>
        <button
          ...
          className="ir-mono text-xs uppercase tracking-wider px-4 py-2"
          style={{ background: mode === "login" ? COLORS.brass : "transparent", color: mode === "login" ? COLORS.ink : COLORS.offwhite }}
        >
          Log in
        </button>
      </div>
      ...
      <form onSubmit={submit} className="space-y-3">
        <input
          ...
          type="email"
          placeholder="Email"
          className="ir-body w-full rounded-md p-3 text-sm outline-none"
        ...
        <input
          ...
          style={{ background: COLORS.ink, color: COLORS.offwhite, border: `1px solid ${COLORS.line}` }}
        />
        {error && <p className="ir-body text-xs" style={{ color: "#e08a8a" }}>{error}</p>}
        <Button className="w-full" disabled={busy}>
          <Mail size={15} /> {busy ? "One sec…" : mode === "signup" ? "Create account" : "Log in"}
        </Button>
      </form>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(10,11,16,0.7)" }}>
      <div className="w-full max-w-md rounded-lg p-6 ir-fade-in" style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}>
        <div className="flex items-center justify-between mb-2">
          <span className="ir-mono text-xs uppercase tracking-wider" style={{ color: COLORS.brass }}>Sign in required</span>
          <button onClick={onClose}><X size={18} style={{ color: "#9aa0b4" }} /></button>
        </div>
        <AuthForm
          onAuthed={onAuthed}
          title="Enter Gospel Connection"
          subtitle="Create a free account or log in to join the conversation."
        />
      </div>
    </div>
  );
...
function Home({ go, onEnterGospel }) {
  return (
    <div className="ir-fade-in space-y-14 pb-10">
      ...
      <div className="mt-10"><GospelConnectionDoor onEnter={onEnterGospel} /></div>
...
function CartPage({ cart, removeFromCart, onCheckoutClick, guideStep }) {
      ...
      {cart.length === 0 ? (
        ...
        <>
          ...
          <div style={{ position: "relative", display: "inline-block" }}>
            <Button className={`w-full md:w-auto ${guideStep === "checkout" ? "ir-guide-pulse" : ""}`} onClick={onCheckoutClick}>
              <CreditCard size={16} /> Checkout
            </Button>
            {guideStep === "checkout" && <GuidePointer text="Tap to checkout" position="bottom" />}
          </div>
        </>
      )}
    </div>
      ...
      {steps.map((s, i) => (
        <React.Fragment key={s.key}>
          <div className="flex flex-col items-center gap-1">
            <div
              className="ir-mono flex items-center justify-center rounded-full"
              style={{
                width: 26, height: 26, fontSize: 12, fontWeight: 700,
                background: i <= currentIndex ? COLORS.brass : COLORS.panelLight,
                color: i <= currentIndex ? COLORS.ink : "#7a8099",
              }}
            >
              {i + 1}
            </div>
            <span className="ir-mono" style={{ fontSize: 10, color: i <= currentIndex ? COLORS.brass : "#7a8099" }}>
              {s.label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div style={{ width: 34, height: 2, marginBottom: 14, background: i < currentIndex ? COLORS.brass : COLORS.line }} />
          )}
        </React.Fragment>
      ))}
    </div>
...
function CheckoutModal({ cart, user, onClose, onComplete }) {
  ...
  const stage = step === "review" ? "review" : step === "account" ? "account" : "payment";

  const goToAccountOrPay = () => {
      ...
      <div className="w-full max-w-md rounded-lg p-6 ir-fade-in" style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}>
        ...
        {step === "review" && (
          <>
            ...
            <Button className="w-full mt-5" onClick={goToAccountOrPay}>
        ...
        <div className="flex items-center gap-2">
          {user && (
            <button
              ...
              style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}
            >
              <LogOut size={15} style={{ color: "#9aa0b4" }} />
            </button>
          )}
          <div style={{ position: "relative" }}>
            <button
              onClick={onCartIconClick}
              className={`relative w-10 h-10 rounded-full flex items-center justify-center ${guideStep === "cart" ? "ir-guide-pulse" : ""}`}
              style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}
            >
            ...
            {guideStep === "cart" && <GuidePointer text="Your item's here!" position="bottom" />}
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (user) {
      loadLibrary(user.uid).then(setLibrary);
    } else {
  ...
  const addToCart = (item) => {
    ...
    setGuideStep((prev) => (prev === "checkout" ? prev : "cart"));
  };
  ...
  const completeCheckout = async () => {
    ...
    setLibrary(next);
    await saveLibrary(user.uid, next);
    setCart([]);
  };
